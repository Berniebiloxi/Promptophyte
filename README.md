...
1. **Read the handbook.** Start with [Level 1](handbook/level-1.md) — a
   ground-up walkthrough of the whole process, using one running example the
   entire way. Keep [Level 2](handbook/level-2.md) open in another tab for
   whenever a specific word (Docker, localhost, linter, and the rest) stops
   you and you want the real explanation behind it.
2. **Paste [`meta-prompt.md`](meta-prompt.md) into your AI coding tool** as
   your first message, with your one- or two-sentence idea dropped into the
   spot marked for it near the top.
3. **Answer its questions.** It will ask about scale, your experience level,
   and then work through the project one or two topics at a time. You can
   reply "accept defaults" at any step. When the interview is done, it hands
   you a set of planning documents and starts building.

## Works with any AI coding agent

Promptophyte isn't tied to one tool or company. The meta-prompt is just
carefully written English. It works with Claude Code, ChatGPT, Gemini,
Cursor, and anything else that can hold a conversation and write files —
use whichever one you already have.
...
```
meta-prompt.md        The core reusable prompt. This is the thing you paste.
